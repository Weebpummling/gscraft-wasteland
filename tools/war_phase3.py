...
EXPECT = {   # each rank's allowed guns (a rank may vary its weapon) and the offhand it must carry
    ...
    "NATO Shield": ({"tacz:m4a1"}, "minecraft:shield"),
    ...
    "RUAF Shield": ({"tacz:ak47"}, "minecraft:shield"),
}
...
check("rank data loaded", counts.get("nato") == 6 and counts.get("ruaf") == 6 and counts.get("scavengers") == 6, counts)
