...
def copy_tree(src: Path, dst: Path, skip=()):
    ...
    for p in sorted(src.rglob("*")):
        ...
        if p.is_file():
            (dst / rel).parent.mkdir(parents=True, exist_ok=True); shutil.copy2(p, dst / rel); out.append(rel.as_posix())
    return out
...
def main():
    ...
    for jar in jars:
        ...
        meta.write_text(f"name = {toml_str(name[:-4])}\nfilename = {toml_str(name)}\nside = \"{side}\"\n\n[download]\nurl = {toml_str(url)}\nhash-format = \"sha512\"\nhash = \"{s512}\"\n{update}", encoding="utf-8", newline="\n")
    ...
    for z in sorted((MC / "tacz").glob("*.zip")):
        ...
        meta.write_text(f"name = {toml_str(z.stem)}\nfilename = {toml_str(z.name)}\nside = \"both\"\n\n[download]\nurl = {toml_str(url)}\nhash-format = \"sha512\"\nhash = \"{sha(z, 'sha512')}\"\n", encoding="utf-8", newline="\n")
    ...
    # plain files: config (server's = the pack's), client-only configs, kubejs (repo), defaultconfigs, servers.dat, tacz-pre.toml
    plain = copy_tree(SERVER_CONFIG, OUT / "config", skip=CONFIG_SKIP)
    plain = ["config/" + p for p in plain]
    for f in CLIENT_CONFIG_EXTRA:
        src = MC / "config" / f
        if src.exists() and not (OUT / "config" / f).exists():
            shutil.copy2(src, OUT / "config" / f); plain.append("config/" + f)
    ...
    for rel in ("servers.dat", "tacz/tacz-pre.toml"):
        src = MC / rel; (OUT / rel).parent.mkdir(parents=True, exist_ok=True); shutil.copy2(src, OUT / rel); plain.append(rel)
    for rel in plain:
    ...
    (OUT / "index.toml").write_text("\n".join(lines), encoding="utf-8", newline="\n")
    (OUT / "pack.toml").write_text(
    ...
    # the Prism instance zip: instance.cfg with the packwiz pre-launch command, bootstrap jar, servers.dat
    ...
    pre = f'"$INST_JAVA" -jar packwiz-installer-bootstrap.jar {RAW}pack.toml'
    cfg += ["OverrideCommands=true", "PreLaunchCommand=" + toml_str(pre), f"notes=GSCraft {VERSION} - the pack installs and updates itself on every launch (packwiz)"]
...
set "PRISM={PRISM_ZIP}"
set "PACK={REL}GSCraft-Instance.zip"
echo.
...
if not exist "%DIR%\\prismlauncher.exe" (
  mkdir "%DIR%" 2>nul
  echo  Downloading Prism Launcher {PRISM_VER} ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol='Tls12'; Invoke-WebRequest -Uri '%PRISM%' -OutFile '%TEMP%\\prism.zip'; Expand-Archive -Path '%TEMP%\\prism.zip' -DestinationPath '%DIR%' -Force" || goto :fail
  if not exist "%DIR%\\portable.txt" type nul > "%DIR%\\portable.txt"
)
echo  Downloading the GSCraft instance ...
powershell -NoProfile -ExecutionPolicy Bypass -Command "[Net.ServicePointManager]::SecurityProtocol='Tls12'; Invoke-WebRequest -Uri '%PACK%' -OutFile '%TEMP%\\GSCraft-Instance.zip'" || goto :fail
...
exit /b 1
"""
    (ASSETS / "GSCraft-Setup.cmd").write_text(cmd, encoding="ascii", newline="\r\n")
