...
2. The vehicle drives the road both ways (its route is the road's two ends) and the infantry follows behind it.
...
squad = c("gscraft fighter @e[type=gscraft:ruaf_soldier,limit=1]")
vx, vz = vpos()
on_road = vx is not None and abs(vz - Z) <= 3.0
off = c(f"gscraft director armour {X} {Y} {Z + 48} superbwarfare:bmp_2 0")
check("the forced roll places a crewed BMP-2 on the road with three infantry in a squad, and nothing off the road",
      road_ok and vehicles == 1 and crews == 1 and soldiers == 3 and on_road and "no road stand" in off,
      f"road block {road_ok}; {out[:40]}; vehicles {vehicles}, crews {crews}, soldiers {soldiers}, at z {vz}; off the road: {off[:30]}")
