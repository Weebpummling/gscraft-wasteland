...
| Lost Cities 7.5.3 logs `Error generating chunk ...: "state" is null` (ChunkDriver.correct) for about 7 % of generated chunks, in the v6 pre-generation and the first lives of v7 | **root cause found and fixed 2026-09-04.** LC's own "stuff" generators (`stuff/chains.json`, `stuff/cobweb.json`) place the palette characters `{` and `\`; the Keerdm apocalypse pack's override of `palettes/common.json` drops those two entries, so the compiled palette returns null and the chunk's city pass aborts at that point (chains, cobwebs and whatever follows are skipped). Fix: the world datapack `build/datapacks/gscraft_lcfix` overrides `common.json` with the two characters restored (and remaps 25 Immersive Weathering block ids the pack lacks, which were harmless - Forge resolves unknown blocks to air). Zero errors from v7 life 5 on. Chunks generated before the fix keep the cosmetic gap. Upstream issues #658 and #754 report the same error without a fix. |
