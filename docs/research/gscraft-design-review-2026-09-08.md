> **Research record, filed 2026-09-13.** the design review of 2026-09-08; its decisions are in the gaps ledger. What was built from it is described in `docs/gscraft-system-2026-09-13.md`; the index is `docs/README.md`.
...
## 4d. Every faction placed by site and dressed by rank (2026-09-09)

`tools/incontrol_areas.py` owns the areas, `tools/spawn_rules.py` the rules, `tools/spawn_probe.py` the
check. They replace two earlier tools that each owned a slice of `spawn.json` and inserted
independently; In Control takes the **first matching rule**, so a block assembled from several insertion
points is a bug waiting to happen, and was one twice.

**Builds are no-spawn ground (owner).** Ten areas deny every hostile: the camp, KROT, the mega-base, the
industrial district, the library, the runway pad and the whole desert-city complex. Nineteen are spawn
ground, and the three big ones are now cut into the places inside them.

### Skadowsky, from a density scan rather than a guess

The hospital, camp, mast and bridge are measured in the design; the station and rail yard never were. So
built blocks were counted per 32 x 32 cell across the sector. Three clusters, north to south — and the
northern one lands exactly on the hospital's documented rectangle, which is the check that the method
works. Rails run north-south through x -960..-928 and past the hospital.

| zone | extent | who |
|---|---|---|
| `sk_hosp` | x -960..-690, z -1344..-1240 | The Infected — a medic's vest, nothing else |
| `sk_town` | x -980..-660, z -1240..-1000 | The Dead — residents, carrying nothing |
| `sk_south` | x -980..-660, z -1000..-760 | Yard Hand — a Gorka jacket and a shovel |

### The power station, from the measured structures
...
`tw_stad` the stadium, `tw_centre` the park and roundabout, `tw_slabs` the west and north-west slab
...
forty pieces; that used two of them and flattened the read at fifty metres, which is the one thing
`gscraft-enemies.md` principle 3 asks the factions to do. Rewritten so kit follows rank and site:
...
| The Dead | the town, the streets, the farm | — | — | — |
| The Drowned | intake works, rail bridge | **copper diving helmet** | **copper backtank** | — |
| Scavenger | Woods and roads | wanderer wrap | wanderer jacket | steel pipe |
...
hospital, a labourer's jacket and a shovel at the yard, and nothing in the town, because they were
civilians. The drowned get the only sealed kit in the pack, which is the one thing that reads as dressed
for water. And the Scavengers deliberately do not match — three different silhouettes across the illager
ranks, none of it issued. That contrast **is** §3.2.

Two §2.1 traps respected: Dragon Rising ships no boots, so nobody is given any; and `cn21`/`cnfast` are
registered as helmets but gate their armour on the chest slot, so they give zero protection on the head
and are not used.
...
four builds returning nothing at all.

With the hold restored the four build probes still pass and the other eighteen produce no entity, which
is the check that the hold suppresses the whole design. Server boots with zero errors from In Control,
Hordes and KubeJS.

`spawn_probe.py` re-runs the lot.
