...
|---|---|---|---|
| 0:00 | Spawn in the yard of the south compound at (−956, 65, −876) (2026-09-12, the compound start) — ~~on the paved junction at (−940, −979), the camp square, in the pocket the team holds inside Skadowsky~~; the team holds the yard, the hall and the brick block, with one open corner to the north-east. **Custom Starting Gear** puts the personal work station, a pistol with one magazine, a flashlight with one battery, a bandage and the survivor's notebook (§6) in the inventory. Night is ten minutes away. | the inventory; that this is a game with guns | Title card: *WASTELAND*. Then the first of Tune's three radio lines, twenty seconds apart: "You're up. Six of us out here — the gate's west, on the bridge." |
| 0:02 | (2026-09-12, the compound start): the hall and the gap — Walker's and Michael's rooms, two Magnum Torches (the yard, the gap's inside corner), the open corner Marshall's first ask closes; the six buildings of the old opening are outside the walls and are taken one by one (start-compound doc §5). ~~Off the square, the pocket: six lit buildings — the north complex, the south complex, Marshall's gatehouse at the bridge's east end and James's signal box on the rail embankment — five Magnum Torches, the~~ river and its one bridge west, the embankment east. The mast stands dead over the rooftops to the east, and the town runs away north past the wire. | the camp is the safe place; the mast on the skyline matters; the town is not ours yet | Nothing spoken. A **sign** on every NPC building: name, role, one line ("WALKER — the yard. Bring me anything with a thread on it."). |
...
| 0:10 | Dusk. ~~Nothing spawns inside the five torches, which cover the pocket at 64 blocks and no further;~~ nothing spawns inside the compound's two torches (the yard, the gap); four more come by stage as the square, the gatehouse, the clinic and the crossing are taken (2026-09-13, reconciliation); the Man From The Fog is heard once, out in the town. | the camp is safe **because of the torches**; the horror starts at the end of the street | — |
...
| 1:05 | James's J1: ~~the hospital, 0.34 km due north (v8; the glass tower and the acacia hall are gone, and so is the settlement), and the level crossing at the camp's east gate [needs measurement].~~ the station and the level crossing — the crossing is itself a building take (`crossing_taken`); the hospital (438 m from the yard) is J-S2's walk (2026-09-13, reconciliation). The first walk out, north up your own street. **Xaero** shows the road; **Ping Wheel** is explained by Tune's line. | the map, waypoints, pings; roads lead somewhere; distance costs time | Tune: "Press M for the map. Middle-mouse pings a spot for everyone." (the only control ever explained in chat) |
| 1:30 | Back at camp with a backpack from W2 (Storage 1). The fifth introduction is done, so Marshall **speaks** for the first time: the camp outline lights up and the map wall is revealed (R1, a datapack function; (2026-09-12, the compound start): the map wall stands on the hall's ground floor, and the reveal comes with `square_taken`); the strongpoint board lights its Skadowsky column — the sector the camp is standing in, state *unknown* — and beside it the hospital, 0.34 km north (v8, design §2.7); the tower chapter appears in the book, and X1's briefing names its five hooks. | the loop exists; the target is the town they are already in; the tower is the long game | Marshall: "We're squatting in someone's town. Clear it and it's ours. Start north — there's a hospital up that road. Ask James what's in it first." | *(the board has no Skadowsky column since R4 (2026-09-13, reconciliation); the takes' torches show the sector's state)*
...
| Safety, torches, the horror | first dusk | ~~five torches covering the pocket and no further~~ two torches at the start, four by stage (2026-09-13, reconciliation); the fog man's sound | nothing | a player who sleeps outside the wire meets the fog man |
| The site ladder | J-S1 → R2 | the strongpoint board (~~seven columns — Skadowsky itself, the five strongpoints and the Woods' outpost —~~ six columns — the five strongpoints and the Woods' outpost; Skadowsky's own state is the takes' torches (ruling R4 (2026-09-13, reconciliation)) — in six colours); later the watchtower banners | one radio line per state change | the marker refused before scouting: Marshall says "James hasn't been. Neither have you." |
...
- **The strongpoint board** ~~at Marshall's gatehouse, at the bridge's east end: seven columns (Skadowsky itself, whose rungs pay the camp's perimeter;~~ on the hall's ground floor inside the compound in Act I (the gatehouse gets it when taken): six columns (ruling R4 (2026-09-13, reconciliation) — Skadowsky's own state is the takes' torches; the five strongpoints — the hospital, the switchyard,
...
~~Early failures are cheap and close to the clinic.~~ Early failures are cheap and close to the compound's hall: the clinic is not held at 0:00, so the hall is the death backstop until `clinic_taken` (2026-09-13, reconciliation). The design keeps them that way and lets them explain:
...
- **Signs and the board** in `camp.py` tier templates; board and banner colour functions
  `gscraft:board_<site>_<state>` for the ~~seven~~ six columns (R4 (2026-09-13, reconciliation): `skadowsky` is not a column) (`skadowsky` and the `hospital_*` stages, which
