...
|---|---|---|---|---|---|
| **I — The town you woke in** | the compound yard (the start), the pocket by takes, then north through Skadowsky (2026-09-13, reconciliation) | 0 – 0.4 km | Skadowsky's own ruins, its station and its level crossing, **the Skadowsky hospital** (0.34 km N of the junction; 438 m from the yard, where the team starts (2026-09-13, reconciliation)) | on foot, inside the sector | 1–2 |
...
| W6 | Hold the yard | II | the camp gate | stage `krot_held`; stage `krot_defended` (the compound's counterattack beaten at the camp gate, at the end of its fortify clock) | W5, Marshall R3 | **Workshop 2** (motor-assembly and mast-section-kit blueprints); **Storage 2**: iron backpack, stack upgrade ×2, magnet upgrade |
...
| W9 | Heavy metal | II–III | KROT | hand in 1 heavy diesel engine, 2 motor assemblies | W8, M-B2, `krot_held` | **Garage 2**: van and truck recipes |
...
| W-A3 | Long guns | II | camp | hand in 2 gun frames, 1 steel frame | W-A1, `krot_defended` | assault-rifle and SMG blueprints; iron sights, extended magazine |
...
| W-M1 | Motor pool | III | camp, the turbine hall, the switchyard | hand in 1 military circuit board, 4 plates, 1 heavy diesel engine; stages `turbine_defended` and `switchyard_defended` | W-A2, W-A4, D4, R5 | **Humvee RWS blueprint** (crafting §2.1; the SW assembling table at yard tier 2 builds it from the kit) |
...
| W-B2 | Second bay | II–III | camp, KROT | hand in 4 steel frames, 32 concrete, 1 heavy diesel engine (the compound respawns them while held) | W-B1, `krot_defended` | **yard tier 2**: two bays, gantry crane, fuel rack, lights; vehicle repair at the bay |
...
| M5 | Hold the intake works | III | the camp gate | stage `intake_held`; stage `intake_defended` (the intake works' counterattack, at the base) | M4, Marshall R5 | **Water 2**: biodiesel chain, fuel cans |
...
| M9 | Hold the turbine hall | III | the camp gate | stage `turbine_held`; stage `turbine_defended` (the turbine hall's counterattack, at the base) | M8, Marshall R5b | **Generator 2**; transformer cores start spawning |
...
| U5 | Hold the switchyard | III | the camp gate | stage `switchyard_held`; stage `switchyard_defended` (the switchyard's counterattack, at the base) | U4, Marshall R4 | **Radio 2**: the contested site's whole countdown on the board; antenna-element blueprint |
...
| U-B2 | Antenna field | III | camp, the switchyard | hand in 4 steel frames, 4 antenna elements, 1 encrypted radio (the switchyard respawns them while held) | U-B1, `switchyard_defended` | **shack tier 2**: antenna field, intel desk (the board's countdown readout is Radio 2's, U5) |
...
| J-T1 | The line | III | the camp's level crossing, the sector's rail yard | walk the rail between the two yards (location, both ends); hand in 16 metal scrap, 4 steel frames — the sleepers and the missing track | S-residential-3, `intake_defended` | stage `train_1`; the level crossing and the yard's points work; the rail yard's loot table switches on (loot §5) |
...
> **Reconciled 2026-09-13:** five take quests must exist in this chapter — R0 the gap (`compound_closed`), and one hand-in each for `square_taken`, `gatehouse_taken`, `clinic_taken` and `crossing_taken`, whose reward runs `/gscraft stage add <alias>` (the mod's clear sets `<id>_cleared`; the quest's stage takes the building and lights its `torch_<name>`). J5's replacement site is still open. H8 gates on H7, so H7 precedes H8 despite the table order in §7A.
...
turbine hall (Rook). ~~The ladder stage ids are unchanged — `novo_*` is the compound's rung, `financial_*` the switchyard's, `plant_*` the intake works', `fr06_*` the turbine hall's — and `residential_*` is renamed `hospital_*` so the sector and the strongpoint do not share a name (§9).~~ The site ids are the mod's (ruling R1): `hospital`, `switchyard`, `intake`, `turbine`, `krot`; rungs `<id>_scouted`, `<id>_looted`, `<id>_held`, `<id>_defended`, `<id>_lost`; building takes add `<id>_cleared` and the readable alias `<id>_taken` (square, gatehouse, clinic, crossing, mast); bosses `<site>_<boss>` (switchyard_gatekeeper); recipes `bp_<recipe>` — `novo`, `financial`, `plant`, `fr06` and `residential` are dead, and the R3–R5b rows below read `krot`, `switchyard`, `intake`, `turbine` (2026-09-13, reconciliation).
...
| R3 | The compound | II | KROT, 2.30 km W over the bridge | place the marker; win the assault (stage `krot_held`) | R2, `hospital_defended`, `novo_looted` | the compound's site guard and Kessler appear; Walker W6 opens |
| R4 | The switchyard | III | the plant's switchyard and admin block, 1.09 km S | place the marker; win the assault (stage `switchyard_held`) | R3, `financial_looted` | Ilya appears; Tune U5 opens |
| R5 | The intake works | III | the plant's cooling intake works, 2.16 km S | place the marker; win the assault (stage `intake_held`) | R4, `krot_defended`, `truck_built`, `plant_looted` | Oksana appears; Michael M5 opens |
| R5b | The turbine hall | III | the plant's turbine hall, 2.06 km S | place the marker; win the assault (stage `turbine_held`) | R5, `fr06_looted` | Rook appears; Michael M9 opens |
...
| D5 | Greenhouse | III | the claim, the intake works | hand in 16 cabbages, 16 onions, 8 cooked meals (Farmer's Delight), 1 industrial pump (irrigation; the intake works respawns them while held) | D3, `intake_defended` | **Farm 2**: greenhouse and irrigation; crops inside the claim grow at double rate (KubeJS random-tick boost) |
| D6 | Rations | III–IV | the claim, the intake works | hand in 32 cooked meals, 1 purification membrane (the intake works respawns them while held) | D5, `intake_defended` | **Farm 3**: hydroponics; the **ration pack** recipe (4 meals → 1 pack, Saturation, stacks 16) — the Act IV run's food |
...
| H8 | The better powder | IV | the outpost, the intake works | hand in 16 nitrate (Oksana's counter, S-plant-2 — pending §7B's re-cut), 8 cotton, 20 powder | H7, S-plant-2 (H8 gates on H7 — H7 comes first, the table's order is wrong; S-plant-2 reads S-intake-2 under R1 (2026-09-13, reconciliation)) | **nitropowder** and **guncotton** blueprints (crafting §5.8) — the long gun's charges; counter LL4 |
...
| G6 | Steel | III | Rook's site T2 | 24 steel ingots poured at Kessler's foundry, the cannon builder at Rook's works (both §7B, pending its re-cut) | S-fr06-2, S-novo-2, G5, `turbine_defended` | the long gun, the quick-firing breech; `gun_steel` |
...
| S-novo-1 | Roof first | II | Kessler, Novo (deferred) | hand in 12 planks, 4 fastener kits, 8 metal scrap | `krot_held` | **Novo tier 1**: the main hall cleared, the yard lit; Kessler's counter (LL1: casting sand, cast-iron nuggets) |
...
| S-financial-1 | The lifts | II | Ilya, the plaza (deferred) | hand in 12 planks, 4 fastener kits, 2 circuit assemblies | `switchyard_held` | **plaza tier 1**: the ground floor cleared, the lifts running; Ilya's counter (LL1: redstone, quartz) |
...
| S-plant-1 | Relight one | III | Oksana, the waterworks (deferred) | hand in 16 planks, 8 metal scrap, 2 filter cartridges | `intake_held` | **waterworks tier 1**: the pump house cleared, one boiler relit; Oksana's counter (LL1: boiler water, packed gunpowder) |
...
| S-fr06-1 | Clear the hall | III | Rook, FR-06 (deferred) | hand in 16 planks, 8 metal scrap, 4 relays | `turbine_held` | **FR-06 tier 1**: the reactor hall floor cleared, the hangar door freed (a Create sliding door); Rook's counter (LL1: steel plates) |
...
(`skadowsky_scouted` → `looted` → `held` → `defended`): holding it extends spawn suppression to the whole sector

> **R11 (2026-09-13, reconciliation):** the sector's rungs are derived from the takes, not claimed: `skadowsky_scouted` = `square_taken`; `skadowsky_held` = the pocket's three other takes held (`gatehouse_taken`, `clinic_taken`, `crossing_taken`; the last take's hand-in sets it); `skadowsky_defended` = `hospital_defended`. No marker is placed on Skadowsky itself.
 and
...
| Group | Stages |
|---|---|
| Site ladder | `<site>_scouted`, `<site>_looted`, `<site>_held`, `<site>_defended`, `<site>_lost` (the counterattack at the base was lost; the site stays held and the wave returns after the next clock) for `hospital` (the Skadowsky hospital — renamed from `residential_*`, 2026-09-07, so the sector and the strongpoint do not share a name), ~~`novo` (KROT), `financial` (the plant's switchyard), `plant` (its cooling intake works), `fr06` (its turbine hall)~~ `krot`, `switchyard`, `intake`, `turbine` (ruling R1 (2026-09-13, reconciliation)); `all_held`. The home sector runs the same ladder on its own axis — `skadowsky_scouted`, `skadowsky_looted`, `skadowsky_held`, `skadowsky_defended` — and pays out perimeter rather than a keeper: `skadowsky_held` extends spawn suppression from the pocket to the whole sector, makes the mast's field camp ground and unlocks NPC building tier 2; `skadowsky_defended` is the first counterattack, fought at the bridge |
...
~~`kubejs stage add`~~ `/gscraft stage add <stage>` (a stage is a player tag; the quest reads it through the mod's advancement `gscraft:stage/<name>` with FTB Quests' native advancement task — ruling R2: no FTB XMod Compat, no KubeJS custom task (2026-09-13, reconciliation)), `function gscraft:tower_stage_N`, `function gscraft:camp_<npc>_<tier>` (the
