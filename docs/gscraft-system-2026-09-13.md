...
| R51 | A fighter fired on sight (eye to eye) at the chest: lowered behind a rise it shot the rise | **the muzzle line**: before a round, the line from the eye to the aim point lowered by `Cover.DROP` (0.35) at both ends must be clear; else the head if that is; else - behind cover or on the way to the lean - hold the round and keep the cover; at the lean or in the open, stand, drop the cover, and no new crouch for `LOW_BLOCKED_TICKS`. `Cover.find` wants a lean clear to the chest as well as the eyes |
| R52 | The counterattack marched to the south compound's corner | `camp.json` `gate` is (-833, -906), just inside the walled compound's north gate. A barred gate (R0) does not seal the compound: `tools/openings.py` finds the wall open round its north-west corner and through two one-wide gaps in the north-east, so a wave goes the long way round (about 300 steps against 52) |
| R53 | The loot was five building tables and an audit that printed; other mods' chests gave diamonds and working guns; sixteen items dropped that nothing used | **the loot design** (`docs/gscraft-loot-design-2026-09-19.md`, owner 2026-09-19): nine building and five site tables from one source (`tools/loot.py`); every foreign `chests/` table replaced at load (`LootRemap`, `remap.json`); the chest-adding global loot modifiers switched off by the datapack (`tools/glm.py`); every item has a use - five new quests (W4, T3, M3, U3, J2), Brass and Tags, five new orders, each hand tool an order's tool; rounds are made from scrap, gunpowder and solvent. **`itemflow.py --gate` must pass and `war_phase46.py` must be 13 of 13 before any loot change ships** |
| R54 | The game made promises it did not keep (the completeness audit, owner 2026-09-19: "nothing left undone besides new features") | **the med kit is a medicine** (`items.json` `"heal": [points, ticks]`, `SliceItems.MedicineItem`): held to use it clears wounds, closes six hearts and - being in `hordes:infection_cures` - cures a bite; nothing else in the economy does (golden apples left with the remap). **Every strongpoint holds containers of its own site table** (the switchyard, the intake works and the turbine hall held none and could never be looted or claimed). The notebook, the field notes and the tooltips say only what is true. `tools/war_phase47.py` holds all of it. Stages a quest sets that nothing reads yet (`workshop_1` ... `intel_1`, the `bp_*`) are flags for features not built, and no text promises anything of them |
