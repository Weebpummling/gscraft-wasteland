...
## 4d. Every faction placed and dressed, probed (2026-09-09)

The whole rule block is generated by `tools/spawn_rules.py` and the areas by `tools/incontrol_areas.py`.
Those replace the two tools that each owned a slice of `spawn.json` and inserted independently: In
Control takes the **first matching rule**, so a block assembled by several tools that each pick their own
insertion point is a bug waiting to happen, and was one twice.

**Builds are no-spawn ground (owner).** KROT is a work in progress and the same goes for every other
player-built and transplanted site. Enemies belong in the base map's own places. Ten areas deny every
hostile: the camp, KROT, the mega-base, the industrial district, the library, the runway pad, and the
whole desert-city complex (hub, plaza, novo, biogen). Six areas are spawn ground: the town, the Woods,
the plant, Skadowsky, the east-bank spine, the farm.

| faction | ground | head | chest | hand |
|---|---|---|---|---|
| Militia | farbank, plant | `us_helmet_pasgt` | `us_chest_iotv` | — |
| Militia Rifleman | farbank, plant | `us_helmet_pasgt` | `us_chest_iotv` | `tacz:type_81` |
| Worker (the Dead) | plant, skad | `armor_faraday_helmet` | `armor_faraday_chestplate` | `crowbar` |
| The Dead | town, farm | — | — | — |
| Scavenger | woods, and everywhere unclaimed | `wandererarmorhelmet` | `wandererarmorchestplate` | `steel_pipe` |
| spiders | skad, town, woods | — | — | — |
| Machines | plant only | — | — | — |

Fourteen probes, all passing: each faction dressed correctly on its own ground, and denied inside KROT,
the mega-base, the camp and the library. A pillager at the Militia's centre is a Rifleman; the same
pillager in the Woods is a Scavenger. Same entity, different faction, decided by where it stood up —
which is §3.2's "the Militia matches, the Scavengers do not", for the price of equipment fields.

### Four things that fail silently, and cost a run each

1. **A gating rule consumes the match.** A rule that only says `default` for a faction in its area stops
   evaluation, so every dressing rule after it never runs. Gate and dress in the *same* rule — actions
   apply whenever a rule matches, whatever the result, so `default` keeps vanilla's spawn logic and still
   dresses what it lets through.
2. **A `maxcount` on a dressing rule re-factions the surplus.** Past the cap the rule stops matching and
   the mob falls through to whatever is next: three of six pillagers at the Militia's own centre came out
   as Scavengers. The cap has to be its own `mincount` **deny** ahead of the dressing, so the surplus is
   refused rather than re-badged.
3. **`helditem` sets the main hand and leaves the off hand to Improved Mobs**, which kept filling it — a
   diamond pickaxe, then flint and steel, then an ender pearl, then a lava bucket, across successive
   runs. Neither the `ARMOR|HELDITEMS` exemption, nor `USEITEM` in the Flag Blacklist, nor the Item Use
   Blacklist stopped it. Writing `HandItems` as NBT sets both slots at once and leaves nothing to fill.
4. **`dim*` is a half-extent, not a width** (§4c), so every area was twice its size and stole mobs from
   later rules.

### Deliberately not included

- **The Torch and the Chemist** (§3.5) need Improved Mobs' item use, which is off while mob block
  destruction is held off. Burning a palisade is the block destruction the hold exists to prevent. They
  return with it.
- **The Horrors** have no rules here: the Knocker, the Eyes and the fog man are governed by their own
  configs (`enemies` §3.4).
- **Skeletons** are "background" in `entities-v8` §2 and keep their existing pre-existing rules; the
  build denies cover them like everything else.

### None of it is live

The hold — one unconditional hostile deny — sits at index 10, ahead of every faction rule and behind the
ten build denies. With it in place a pillager summoned at the Militia's centre produces no entity at all.
Removing that one rule switches the whole design on. Server boots with zero errors from In Control,
Hordes and KubeJS.
