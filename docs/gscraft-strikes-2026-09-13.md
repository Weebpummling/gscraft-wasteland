...
**The global cooldown** is one for all three: five minutes (`strike.cooldown_ticks`) from any call. The console's
`/gscraft strike status|reset` reads and clears it. Every number above is a setting (`strike.*`).
