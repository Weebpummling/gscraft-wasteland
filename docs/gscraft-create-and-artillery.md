...
| G7 | Shells | III–IV | the plaza T2 | S-financial-2, G6 | mechanical crafters: 4 HE shells, 2 AP shells, 2 shrapnel; fuzes: impact and proximity (the mod's own recipes; there is no "fuze head" item) | shells and fuzes as station orders; the smoke shell for the Woods |
...
- `gscraft-camp-spec.md`: `site.py` (templates per site and tier, keeper summons) beside `camp.py`; the gun pit on the
  plateau's east bluff in the camp's tier-0 layout.
