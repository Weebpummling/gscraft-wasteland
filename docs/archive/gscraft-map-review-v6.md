> **Archived 2026-09-13.** the v6 review. Nothing in this file is current; it is kept as the record. The living description of the game is `docs/gscraft-system-2026-09-13.md`; the index is `docs/README.md`.
...
| Lost Cities 7.5.3 logs `Error generating chunk ...: "state" is null` (ChunkDriver.correct) for about 7 % of generated chunks, in the v6 pre-generation and the first lives of v7 | **root cause found and fixed 2026-09-04.** LC's own "stuff" generators (`stuff/chains.json`, `stuff/cobweb.json`) place the palette characters `{` and `\`; the Keerdm apocalypse pack's override of `palettes/common.json` drops those two entries, so the compiled palette returns null and the chunk's city pass aborts at that point (chains, cobwebs and whatever follows are skipped). Fix: the world datapack `build/datapacks/gscraft_lcfix` overrides `common.json` with the two characters restored (and remaps 25 Immersive Weathering block ids the pack lacks, which were harmless - Forge resolves unknown blocks to air). Zero errors from v7 life 5 on. **The affected chunks are not cosmetic: the chunk's whole Lost Cities content is skipped, leaving a bare-terrain hole in the city** (verified: logged error chunks hold 0 city blocks, their neighbours hundreds). About a third of the city chunks in the deployed v6 world are such holes; v7 regenerates everything generated before the fix, so only the transplanted district's own surroundings keep them. Upstream issues #658 and #754 report the same error without a fix. |
