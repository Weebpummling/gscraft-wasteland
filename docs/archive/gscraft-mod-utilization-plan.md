> **Archived 2026-09-13.** the mod utilization plan; superseded by gscraft-mod-capabilities.md. Nothing in this file is current; it is kept as the record. The living description of the game is `docs/gscraft-system-2026-09-13.md`; the index is `docs/README.md`.
...
| C6 | Immersive Engineering | `gunpart_barrel` + `gunpart_drum` + `gunpart_hammer` → `revolver` | Act III · W-A4 (quests §2), not the audit's W-A2: the parts drop only from NATO at FR-06 (enemies §6) · Walker's card: 3 parts + 1 gun frame | NATO's own sidearm, taken from them | tooltip | one order line | C |
...
| C6 the revolver card at W-A4 | **yes** (the parts only drop from NATO on the far bank, Act III) | crafting §5.2, quests W-A4 |
