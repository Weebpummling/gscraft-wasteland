...
| **The Hordes — infection** | zombie hits infect; infection kills unless cured (the horde event ships disabled and stays off, B6) | not designed with | the medical function's reason to exist: Tony cures infection at the clinic from T1, the med kit cures in the field from Medical 2. Ties the residential block (blood bags) to the mechanic |
